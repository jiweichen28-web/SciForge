  ...
  return Object.freeze({
    ...
    async action(handleId, input) {
      ...
      if (name === 'click' || name === 'left_click' || name === 'right_click' || name === 'double_click') {
        const [x, y] = await targetCoordinate(value.page, action.coordinate)
        ...
        if (!chord) {
          throw new CdpAdapterDriverError('INVALID_ARGUMENT', 'CDP key action requires keys.', true)
        }
        const beforeUrl = value.page.url()
        const beforeSemanticTree = await pageSemanticTree(value.page)
    ...
    async shutdown() {
      ...
      browsers.clear()
      const closed = await Promise.allSettled(connected.flatMap((result) => (
        result.status === 'fulfilled' ? [result.value.close()] : []
      )))
      const failures = closed.flatMap((result) => result.status === 'rejected' ? [result.reason] : [])
      if (failures.length > 0) {
        throw new AggregateError(failures, 'CDP browser transports did not fully disconnect.')
      }
    }
  })
}
...
async function targetCoordinate(page: Page, value: unknown): Promise<[number, number]> {
  const [normalizedX, normalizedY] = coordinate(value)
  const viewport = asRecord(await evaluateCdpReadback(page, CDP_CSS_VIEWPORT_EXPRESSION))
  const width = finiteNumber(viewport.width, 1)
  const height = finiteNumber(viewport.height, 1)
  return [normalizedX / 1000 * width, normalizedY / 1000 * height]
}
...
export async function startComputerUseCdpAdapter(options: Readonly<{
  ...
  return Object.freeze({
    ...
    async close() {
      ...
      if (errors.length > 1) throw new AggregateError(errors, 'CDP adapter cleanup was incomplete.')
    }
  })
}
  ...
  try {
    ...
    send(response, httpError ? error.status : driverError ? 409 : 400, {
      ...
      error: {
        ...
        ...(path === '/v1/action' && !['STALE_OBSERVATION', 'ACTION_UNSUPPORTED', 'INVALID_ARGUMENT']
          .includes(httpError || driverError ? error.code : classifyError(error))
...
function coordinate(value: unknown): [number, number] {
  if (!Array.isArray(value) || value.length !== 2) {
    throw new CdpAdapterDriverError(
      'INVALID_ARGUMENT', 'CDP click requires one x/y coordinate.', true
    )
  }
  const output: [number, number] = [
    requiredCoordinatePart(value[0]), requiredCoordinatePart(value[1])
  ]
  if (output.some((part) => part < 0 || part > 1000)) {
    throw new CdpAdapterDriverError(
      'INVALID_ARGUMENT',
      'CDP click coordinate must stay inside the normalized target viewport.',
      true
    )
  }
  return output
...
function requiredCoordinatePart(value: unknown): number {
  const parsed = Number(value)
  if (!Number.isFinite(parsed)) {
    throw new CdpAdapterDriverError('INVALID_ARGUMENT', 'CDP click coordinate must be finite.', true)
  }
  return parsed
}
