...
describe('Computer Use Web Task Lab', () => {
  ...
  it('publishes authoritative success oracles that match rendered results', () => {
    ...
    assert.equal(multipage.oracle.status, 'workflow-step-3')
    const download = WEB_TASKS.find(({ id }) => id === 'epsilon')
    assert.ok(download)
    assert.equal(download.oracle.status, 'download-served')
    assert.equal(download.oracle.downloadRequestDelta, 1)
